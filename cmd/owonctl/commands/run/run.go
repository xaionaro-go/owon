// Package run implements the owonctl run command.
//
// Example: `owonctl run` starts continuous acquisition and prints the result as JSON.
package run
...
func NewCommand(config *owonctl.ClientConfig) *cobra.Command {
	...
	command := &cobra.Command{Use: "run", Short: "Start continuous acquisition", Args: cobra.NoArgs, RunE: handler.run}
...
func (handler *commandHandler) run(
	...
	defer cancel()
	ctx = belt.WithField(ctx, "command", "run")
	...
	logger.Debugf(ctx, "executing run")
	response, err := connection.Client().Run(ctx)
	if err != nil {
		return err
	}
